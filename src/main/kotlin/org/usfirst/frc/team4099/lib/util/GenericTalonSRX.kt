...
class GenericTalonSRX(override val id: Int, override val timeout: Int) : GenericSmartMotorController, TalonSRX(id) {
    ...
    override var selectedEncoder: GenericSmartMotorController.EncoderType = GenericSmartMotorController.EncoderType.NONE
        set(value) {
            when (value) {
                GenericSmartMotorController.EncoderType.ABSOLUTE_ANALOG -> super.configSelectedFeedbackSensor(FeedbackDevice.Analog)
                GenericSmartMotorController.EncoderType.ABSOLUTE_PWM -> super.configSelectedFeedbackSensor(FeedbackDevice.PulseWidthEncodedPosition)
                GenericSmartMotorController.EncoderType.QUADRATURE -> super.configSelectedFeedbackSensor(FeedbackDevice.QuadEncoder)
    ...
    override var velocityMeasPeriod: GenericSmartMotorController.VelocityMeasPeriod = GenericSmartMotorController.VelocityMeasPeriod.MS_50
        set(value) {
            when (value) {
                GenericSmartMotorController.VelocityMeasPeriod.MS_1 -> super.configVelocityMeasurementPeriod(VelocityMeasPeriod.Period_1Ms, timeout)
                GenericSmartMotorController.VelocityMeasPeriod.MS_2 -> super.configVelocityMeasurementPeriod(VelocityMeasPeriod.Period_2Ms, timeout)
                GenericSmartMotorController.VelocityMeasPeriod.MS_5 -> super.configVelocityMeasurementPeriod(VelocityMeasPeriod.Period_5Ms, timeout)
                GenericSmartMotorController.VelocityMeasPeriod.MS_10 -> super.configVelocityMeasurementPeriod(VelocityMeasPeriod.Period_10Ms, timeout)
                GenericSmartMotorController.VelocityMeasPeriod.MS_20 -> super.configVelocityMeasurementPeriod(VelocityMeasPeriod.Period_20Ms, timeout)
                GenericSmartMotorController.VelocityMeasPeriod.MS_25 -> super.configVelocityMeasurementPeriod(VelocityMeasPeriod.Period_25Ms, timeout)
                GenericSmartMotorController.VelocityMeasPeriod.MS_50 -> super.configVelocityMeasurementPeriod(VelocityMeasPeriod.Period_50Ms, timeout)
                GenericSmartMotorController.VelocityMeasPeriod.MS_100 -> super.configVelocityMeasurementPeriod(VelocityMeasPeriod.Period_100Ms, timeout)
            }
            field = value
        }
    ...
    override fun set(mode: GenericSmartMotorController.ControlMode, outputValue: Double) {
        when (mode) {
            ...
            GenericSmartMotorController.ControlMode.VELOCITY -> set(ControlMode.Velocity, velocityToRaw(outputValue).toDouble())
            GenericSmartMotorController.ControlMode.POSITION -> set(ControlMode.Position, positionToRaw(outputValue).toDouble())
            GenericSmartMotorController.ControlMode.PROFILED -> set(ControlMode.MotionMagic, positionToRaw(outputValue).toDouble())
        }
    }
    ...
    override fun setControlFramePeriod(frame: GenericSmartMotorController.ControlFrame, periodMs: Int) {
        when (frame) {
            GenericSmartMotorController.ControlFrame.GENERAL -> setControlFramePeriod(ControlFrame.Control_3_General, periodMs)
            GenericSmartMotorController.ControlFrame.ADVANCED -> setControlFramePeriod(ControlFrame.Control_4_Advanced, periodMs)
        }
    }

    override fun setStatusFramePeriod(frame: GenericSmartMotorController.StatusFrame, periodMs: Int) {
        when (frame) {
            GenericSmartMotorController.StatusFrame.GENERAL -> setStatusFramePeriod(StatusFrame.Status_1_General, periodMs, timeout)
            GenericSmartMotorController.StatusFrame.PRIMARY_FEEDBACK -> setStatusFramePeriod(StatusFrameEnhanced.Status_2_Feedback0, periodMs, timeout)
            GenericSmartMotorController.StatusFrame.MISC -> setStatusFramePeriod(StatusFrame.Status_6_Misc, periodMs, timeout)
            GenericSmartMotorController.StatusFrame.COMM_STATUS -> setStatusFramePeriod(StatusFrame.Status_7_CommStatus, periodMs, timeout)
            GenericSmartMotorController.StatusFrame.TARGETS -> setStatusFramePeriod(StatusFrame.Status_10_Targets, periodMs, timeout)
            GenericSmartMotorController.StatusFrame.SECONDARY_FEEDBACK -> setStatusFramePeriod(StatusFrame.Status_12_Feedback1, periodMs, timeout)
            GenericSmartMotorController.StatusFrame.PRIMARY_PID -> setStatusFramePeriod(StatusFrameEnhanced.Status_13_Base_PIDF0, periodMs, timeout)
            GenericSmartMotorController.StatusFrame.SECONDARY_PID -> setStatusFramePeriod(StatusFrameEnhanced.Status_14_Turn_PIDF1, periodMs, timeout)
        }
    }
    ...
    override fun setForwardLimitSwitch(source: GenericSmartMotorController.LimitSwitchSource, normal: GenericSmartMotorController.LimitSwitchNormal) {
        ...
        when (source) {
            GenericSmartMotorController.LimitSwitchSource.CONNECTED -> configForwardLimitSwitchSource(LimitSwitchSource.FeedbackConnector, limitSwitchNormal, timeout)
            GenericSmartMotorController.LimitSwitchSource.REMOTE_CANIFIER -> configForwardLimitSwitchSource(LimitSwitchSource.RemoteCANifier, limitSwitchNormal, timeout)
            GenericSmartMotorController.LimitSwitchSource.REMOTE_SRX -> configForwardLimitSwitchSource(LimitSwitchSource.RemoteTalonSRX, limitSwitchNormal, timeout)
        }
    }
    ...
    override fun setReverseLimitSwitch(source: GenericSmartMotorController.LimitSwitchSource, normal: GenericSmartMotorController.LimitSwitchNormal) {
        ...
        when (source) {
            GenericSmartMotorController.LimitSwitchSource.CONNECTED -> configReverseLimitSwitchSource(LimitSwitchSource.FeedbackConnector, limitSwitchNormal, timeout)
            GenericSmartMotorController.LimitSwitchSource.REMOTE_CANIFIER -> configReverseLimitSwitchSource(LimitSwitchSource.RemoteCANifier, limitSwitchNormal, timeout)
            GenericSmartMotorController.LimitSwitchSource.REMOTE_SRX -> configReverseLimitSwitchSource(LimitSwitchSource.RemoteTalonSRX, limitSwitchNormal, timeout)
        }
    }
